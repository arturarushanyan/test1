...
//            baseUrl: 'http://127.0.0.1/docs',
        ...
        spritepacker2: {
            sport_icons: { // flags spritre generation
                options: {
                    template: '../css/sport_icons.sprite.tpl',
                    destCss: '../css/sport_icons.css',
                    baseUrl: '../images/',
                    padding: 5
                },
                files: {
...
//    grunt.loadNpmTasks('grunt-inline-angular-templates');
    ...
    grunt.config.set('deployServerPath', grunt.option('deploy-path') || serverOptions.paths[grunt.config.get('skinConfig')]);
    grunt.config.set('deployServerKey', grunt.option('deploy-key') || serverOptions.keys[grunt.config.get('skinConfig')] || serverOptions.keys._default);
    grunt.config.set('deployServerHost', grunt.option('deploy-host') || serverOptions.hosts[grunt.config.get('skinConfig')] || serverOptions.hosts._default);
    ...
    grunt.config.set('defaultCssFileList', ['soccer-control.css', 'tennis-control.css', 'games-animations-classic.css', 'main.css', 'media.css', 'flags.css','sport_icons.css', 'transitions.css', 'lib/introjs.min.css', 'lib/rg-slider.min.css', 'lib/rzslider.min.css', 'lib/hopscotch.min.css', 'lib/barcode.css'])
...
//    grunt.registerTask('default-inline-templates',     ['closure-compiler', 'processhtml', 'strip', 'inline_angular_templates', 'copy', 'clean:build-temp']);
    ...
    grunt.registerTask('default',                      ['gitinfo', 'load-additional-modules', 'create-conf', 'load-css-filelist', 'ngtemplates', 'load-translations', 'replace:removeContextAttr', 'replace:changeAppVersion', 'closure-compiler', 'cssmin', 'strip', 'processhtml', 'replace:xDomainFrameUrl', 'replace:htmlPlaceholders', 'no-cdn-libs', 'replace:noCdnLibs', 'copy:main', 'clean:build-temp']);
    ...
    grunt.registerTask('build-and-deploy-css',              ['create-css', 'shell:makeGzipFiles', 'deploy-css']);
    grunt.registerTask('build-and-deploy-css-and-images',   ['create-css', 'shell:makeGzipFiles', 'deploy-css-images']);
    grunt.registerTask('build-and-deploy-css-and-fonts',    ['create-css', 'shell:makeGzipFiles', 'deploy-css-fonts']);
