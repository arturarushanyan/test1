...
BettingModule.controller('betSlipController', ['$q', '$scope', '$rootScope', '$filter', 'Config', 'Zergling', 'Storage', 'Translator', '$location', '$route', '$window', '$injector', 'analytics', 'DomHelper', 'Utils', 'partner', 'TimeoutWrapper', 'UserAgent', function ($q, $scope, $rootScope, $filter, Config, Zergling, Storage, Translator, $location, $route, $window, $injector, analytics, DomHelper, Utils, partner, TimeoutWrapper, UserAgent) {
    ...
    if (Config.betting.rememberAutoAcceptPriceChangeSettings && Storage.get('autoAcceptSettings')) {
    ...
    $scope.$watch('acceptPriceChanges', function () {
        if (Config.betting.rememberAutoAcceptPriceChangeSettings) {
                ...
                angular.forEach(game.market, function (market) {
                    angular.forEach(market.event, function (event) {
                        if (b.eventId === event.id) {
                            ...
                            b.title = (game.displayTitle ?  game.displayTitle + " " : "") + game.team1_name + (game.team2_name ? ' - ' + game.team2_name : '');
    ...
    function subscribeToBetslipEvents() {
        ...
        Zergling
            .subscribe({
                'source': 'betting',
                'what': {
                    'game': ['id', 'is_blocked', 'team1_name', 'team2_name'],
                    'market': ['base', 'type', 'name'],
    ...
    $scope.getMaxBet = function getMaxBet(betEvents) {
        ...
        var processMaxBetResults = function (result) {
            if (angular.isArray(betEvents)) {
                $scope.lustMaxBetResult = $scope.betSlip.stake = parseFloat(result.result);
            } else {
                betEvents.singleStake =  parseFloat(result.result);
            }
        };
        ...
        angular.forEach($scope.betTypes, function(betType) {
            if (betType.value === 1) {
                if ($scope.betSlip.bets.length === 1 && $scope.betSlip.type.value === 2 || ($scope.betSlip.bets.length === 0)) {
                    ...
                    $scope.betSlip.superbetSelector = true;
                }
            }
        });
    ...
    function addBet(event, data) {
        if (!addBetInProgress && !$scope.isBetsInProgress && $scope.quickBet.status !== 'processing' && data.event.price  !== 1 && !data.game.is_blocked && (Config.betting.enableHorseRacingBetSlip || (!Config.betting.enableHorseRacingBetSlip && data.event.price  !== undefined))) {//temporary reject add events without price into betslip
    ...
    function getBetSlipError(result) {
        var code = 'message_' + Math.abs(parseInt(result.code || result.result || 99, 10)).toString();
        return Translator.get("In order to use your bonus balance, odds should be at least 1.5");
    }
    ...
    $scope.placeBookingBet = function placeBookingBet() {
        ...
        $scope.betSlip.generalBetResult = "";
        analytics.gaSend('send', 'event', 'betting', 'PlaceBookingBet ' + (Config.main.sportsLayout) + ($scope.env.live ? '(LIVE)' : '(PM)'), {'page': $location.path(), 'eventLabel': ({1: 'single', 2: 'express', 3: 'system'}[$scope.betSlip.type.value])});
        var requests = [];
        var currentBets;
        ...
        angular.forEach(requests, function (request) {
            ...
            var processBetResults = function (result) {
                ...
                if (result.result === 0) {
                    ...
                    analytics.gaSend('send', 'event', 'betting', 'AcceptedBookingBet ' + (Config.main.sportsLayout) + ($scope.env.live ? '(LIVE)' : '(PM)'), {'page': $location.path(), 'eventLabel': ({1: 'single', 2: 'express', 3: 'system'}[$scope.betSlip.type.value])});
    ...
    $scope.placeQuickBet = function placeQuickBet(data) {
        ...
        if ($scope.quickBetEnabled && !$scope.hasWrongStakes && !$scope.betSlip.hasEmptyStakes && $scope.env.authorized && !$scope.isBetsInProgress && ($scope.betSlip.totalStake <= $scope.profile.calculatedBalance)) {
                ...
                angular.forEach(bets, function (bet) {
                    if ((!$scope.betSlip.thereAreDeletedEvents && !$scope.thereIsPriceChange() && !isNaN(parseFloat(bet.singleStake))) || forFreeBet) {
                        data = {
                            ...
                            'bets': [
                                {'event_id': bet.eventId, 'price': bet.oddType === 'sp' ? -1 : $scope.betSlip.betterOddSelectionMode ? bet.betterPrice : bet.price}
                            ]
                        };
                        if (!forFreeBet) {
    ...
    $scope.placeBet = function placeBet(singleBetEvent) {
        ...
        analytics.gaSend('send', 'event', 'betting', 'PlaceBet ' + (Config.main.sportsLayout) + ($scope.env.live ? '(LIVE)' : '(PM)'), {'page': $location.path(), 'eventLabel': ({1: 'single', 2: 'express', 3: 'system'}[$scope.betSlip.type.value])});
        ...
        angular.forEach(requests, function (request) {
            ...
            var processBetResults = function (result) {
                ...
                if (result.result === 'OK') {
                    ...
                    haveAcceptedEvent = true;
                    analytics.gaSend('send', 'event', 'betting', 'AcceptedBet ' + (Config.main.sportsLayout) + ($scope.env.live ? '(LIVE)' : '(PM)'), {'page': $location.path(), 'eventLabel': ({1: 'single', 2: 'express', 3: 'system'}[$scope.betSlip.type.value])});
                    ...
                    //superbet for spring=================================
                    if (Config.main.GmsPlatform && $scope.acceptPriceChanges == -1) {
            ...
            switch ($scope.betSlip.type.value) {
                case 1://single
                    ...
                    if (!isNaN(parseFloat(bet.singleStake)) && parseFloat(bet.singleStake) > 0 && !bet.deleted && bet.oddType !== 'sp' && !bet.blocked) {
                        ...
                        var realPrice = $scope.betSlip.betterOddSelectionMode ? bet.betterPrice : bet.price;
        ...
        switch ($scope.betSlip.type.value) {
            ...
            case 2:
                ...
                if ($scope.betSlip.stake && ($scope.freeBetSelected() || $scope.bonusBet.enabled)) {
                    return setMaxWinLimit(Math.round((totalOdd * $scope.betSlip.stake - $scope.betSlip.stake) * 100 || 0) / 100);
                }
            ...
            case 3:
                if ($scope.betSlip.bets.length > 2) {
                    ...
                    if ($scope.betSlip.stake && $scope.bonusBet.enabled) {
                        return setMaxWinLimit(Math.round((bankerTotalPrice * tempResult.win - $scope.betSlip.stake) * 1000 || 0) / 1000);
                    }
                    return setMaxWinLimit(Math.round((bankerTotalPrice * tempResult.win) * 1000 || 0) / 1000);
                }
                break;
            default:
                return 0;
    ...
    $scope.gotoBetGame = function gotoBetGame(gamePointer) {
        ...
        var neededPath = Utils.getPathAccordintToAlias(gamePointer.alias);
    ...
    $scope.betFromKeyboard = function betFromKeyboard($event) {
        ...
        if ($event.keyCode == 13 && !($scope.thereIsPriceChange() || !$scope.betSlip.isBettingAllowed || !$scope.env.authorized || $scope.isBetsInProgress || $scope.betSlip.thereAreDeletedEvents || ($scope.env.authorized && !$scope.freeBet.enabled && $scope.betSlip.totalStake > $scope.profile.balance + $scope.profile.bonus_balance + $scope.profile.bonus_win))) {
            $scope.placeBet();
        }
    };
    ...
    $scope.showInfoAboutBetterBet = function showInfoAboutBetterBet () {
        $rootScope.$broadcast("globalDialogs.addDialog", {
            type: 'info',
            title: 'Information',
