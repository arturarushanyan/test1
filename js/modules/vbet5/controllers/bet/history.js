        ...
        angular.forEach(allBets, function (betData) {
            if (betData.type == '1' || betData.type == '2') {
                ...
                if (!hasInactiveEvents) {
                    ...
                    betData.calculatedCashout =  ((betData.k / newCoef) * betData.amount * amountCoef).toFixed(Math.abs($rootScope.currency.rounding));
...
//             'with_pool_bets': true
    ...
    $scope.gotoBetGame = function gotoBetGame(betEvent) {
        if (GameInfo.VIRTUAL_SPORT_IDS.indexOf(parseInt(betEvent.sport_id, 10)) === -1 && $rootScope.gamePointers[betEvent.game_id]) {
    ...
    $scope.calculateProfit = function calculateProfit(isThisMonth) {
        ...
        angular.forEach(betHistory, function (value) {
            ...
            switch (value.outcome) {
                ...
                case 5:
                case '5':
                    result += parseFloat(value.payout);
                    break;
            }
        });

        $scope.profit.show = true;
        $scope.profit.result = Math.round(parseFloat(result) * 10) / 10;
