...
VBET5.controller('mixedMyBetsCtrl', ['$scope', '$controller', 'Config', '$rootScope', 'Moment', 'Utils', '$location', function($scope, $controller, Config, $rootScope, Moment, Utils, $location) {
    ...
    function initScope() {
        ...
        $scope.datePickerFormat = Config.main.layoutTimeFormat[Config.main.sportsLayout] == 'MM/DD' ? 'MM/dd/yyyy': 'dd/MM/yyyy';
    ...
    $scope.loadMixedBetHistory = function loadMixedBetHistory(product) {
        if (product) {
            ...
            if (product  === 'Casino') {
                $scope.requestData.dateFrom = Moment.moment($scope.today).add(-1, "w").format("YYYY-MM-DD");
            ...
            case 'from':
                ...
                if ($scope.productMode === "Sportsbook") {
                    ...
                    if (Moment.get($scope.requestData.dateFrom).add(1, "w").isAfter($scope.today)){
                        ...
                        $scope.requestData.dateTo = Moment.get($scope.requestData.dateFrom).add(1, "w").format("YYYY-MM-DD");
                        $scope.datePickerLimits.maxToDate = Moment.moment($scope.requestData.dateFrom).add(1, "w").format("YYYY-MM-DD");
                    }
                }
