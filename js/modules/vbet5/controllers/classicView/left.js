...
angular.module('vbet5.betting').controller('classicViewLeftController', ['$rootScope', '$scope', '$location', '$window', '$filter', '$route', 'DomHelper', 'Utils', 'Zergling', 'ConnectionService', 'GameInfo', 'Storage', 'Config', 'Translator', 'analytics', 'TimeoutWrapper', function ($rootScope, $scope, $location, $window, $filter, $route, DomHelper, Utils, Zergling, ConnectionService, GameInfo, Storage, Config, Translator, analytics, TimeoutWrapper) {
    ...
    function openInitialDefaultGameIfNeeded() {
    ...
    $scope.subscribeToAllGameCounts = function subscribeToAllGameCounts(type) {
        ...
        if ($scope.customSportAliasFilter) {
            ...
            request.where.sport.alias = {'@in': $scope.customSportAliasFilter};
        }
    ...
    function loadLeftMenuLive() {
        var request = {
            ...
            'what': {
                ...
                game: ['id', 'start_ts', 'team1_name', 'team2_name', 'type', 'info', 'events_count', 'markets_count', 'is_blocked', 'stats', 'tv_type', 'video_id', 'video_id2', 'video_id3', 'is_stat_available', 'show_type']
            },
            'where': {'game': {'type': 1}}
        };
            ...
            connectionService.subscribe(
                {
                    ...
                    'where': {'game': {'id': game.id, 'not_in_sportsbook': 0}}
                },
                function (result) {
                    if (!result) {
                        return;
    ...
    var additionalLeftMenuItems = {
        ...
        FAVORITE_COMPETITIONS: {
            id: -12,
            innumerable: true,
            name: Config.main.enableOlympicGames ? Translator.get('Olympic Games') : Translator.get('Quick Links'),
    ...
    function addPopularCompetitionsInMenu() {
        Zergling
            .get({'source': 'betting', 'what': {'competition': '@count'}, 'where': {'competition': {'favorite': true},'game':{not_in_sportsbook:0}}})
            .then(function (result) {
                console.log("addPopularCompetitionsInMenu", result);
                if (Config.main.showFavoriteCompetitions && result.data.competition) {
    ...
    $scope.expandLeftMenuPrematchSport = function  expandLeftMenuPrematchSport(sport, keepItOpen, doChainingOpen, leaveMenuOpen) {
        ...
        if (sport.id === additionalLeftMenuItems.VIRTUAL_SPORT_VIRTUALS.id) {
            if (Config.main.enableSportsListInVirtualSports) {
    ...
    function updateLeftMenuFavoriteCompetitions(sport, data) {
        TimeoutWrapper(function () {
            ...
            angular.forEach(favoriteSports, function (favoriteSport) {
                angular.forEach(favoriteSport.competition, function (favoriteCompetition) {
