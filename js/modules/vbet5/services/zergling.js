...
//                         }
     ...
     */
    var restrictionPromise, loginProccesWatcher, waitingPromise;
    function checkLoggedInStatus() {
        if (Config.env.authorized) {
            if (restrictionPromise) {
                $timeout.cancel(restrictionPromise);
            }
            if (loginProccesWatcher) {
                loginProccesWatcher();
            }
            if (waitingPromise) {
                $timeout.cancel(waitingPromise);
            }

            waitingPromise = $timeout(function () {
                if ($rootScope.loginInProgress) {
                    loginProccesWatcher = $rootScope.$watch('loginInProgress', function () {
                        if (!$rootScope.loginInProgress) {
                            $timeout.cancel(restrictionPromise);
                            restrictionPromise = null;
                            loginProccesWatcher();
                            loginProccesWatcher = null;
                        }
                    });
                    restrictionPromise = $timeout(function () {
                        loginProccesWatcher();
                        loginProccesWatcher = null;
                        restrictionPromise = null;
                        //@TODO in this case we must disable view until login process finish
