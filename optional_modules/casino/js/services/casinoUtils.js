...
CASINO.service('casinoUtils', ['$rootScope', '$window', '$sce', '$location', 'Storage', 'DomHelper', 'Zergling', 'Config', 'CConfig', 'Translator', function ($rootScope, $window, $sce, $location, Storage, DomHelper, Zergling, Config, CConfig, Translator) {
    'use strict';
    var casinoUtils = {};
    ...
    casinoUtils.openPopUpWindow = function openPopUpWindow(scope, id) {
        ...
        if (CConfig.main.providersThatWorkWithSwarm.indexOf(game.gameProvider) !== -1 || game.gameID === CConfig.belote.gameID) {
    ...
    function addCurrentGame(scope, gameInfo, gameUrl, toAddIndex) {
        if (CConfig.main.providersThatWorkWithSwarm.indexOf(gameInfo.game.gameProvider) !== -1 || gameInfo.game.gameID === CConfig.belote.gameID) {
